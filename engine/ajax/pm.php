...
if ($_POST['action'] == "send_pm") {
	...
	$response = "<form  method=\"post\" name=\"dle-send-pm\" id=\"dle-send-pm\"><input type=\"hidden\" name=\"pm_name\" id=\"pm_name\" value=\"{$name}\">";
...
HTML;
	...
	if( $user_group[$member_id['user_group']]['captcha_pm'] ) {
		...
		if ( $config['allow_recaptcha'] ) {
			...
			$response .= <<<HTML
	<div id="dle_pm_recaptcha" style="padding-top:5px;height:78px;"></div><input type="hidden" name="pm_recaptcha" id="pm_recaptcha" value="1">
...
HTML;
	...
	echo "<div id='dlesendpmpopup' title='{$lang['send_pm']} {$name}' style='display:none'>{$response}</div>";
	die();
