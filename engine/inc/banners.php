...
if( isset( $_REQUEST['rubric'] ) ) $rubric = intval( $_REQUEST['rubric'] );
else $rubric = 0;
...
if( $_POST['action'] == "doadd" ) {
	...
	header( "Location: ?mod=banners&rubric=".$rub_id );
	die();
...
if( $_POST['action'] == "addrubric" ) {
	...
	header( "Location: ?mod=banners&rubric=".$rubric );
	die();
...
HTML;
	...
	$db->query( "SELECT * FROM " . PREFIX . "_banners WHERE rubric='{$rubric}' ORDER BY id DESC" );
