...
<div class="panel panel-default">
  <div class="panel-heading">
	...
	<div class="heading-elements">
		<ul class="icons-list">
			<li><a href="#" data-toggle="modal" data-target="#advancedadd"><i class="fa fa-user-plus position-left"></i>{$lang['user_auser']}</a></li>
		</ul>
	</div>
  </div>
...
HTML;
	...
	if( ! $id ) {
		if ($_GET['popup'] == "yes") die( $lang['user_nouser'] );
		else msg( "error", $lang['user_err'], $lang['user_nouser'] );
	}

	if( $id == 1 ) {
		
		if ($_GET['popup'] == "yes") die( $lang['user_undel'] );
	...
	if( $member_id['user_id'] == $row['user_id']) {
		...
		if ($_GET['popup'] == "yes") die( $lang['no_self'] );
