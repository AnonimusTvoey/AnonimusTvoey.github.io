...
<!--
		...
		$(function(){
			...
			$('#check_updates').click(function() {
				...
				$.get("engine/ajax/controller.php?mod=updates&versionid={$config['version_id']}&user_hash={$dle_login_hash}", function( data ){
					HideLoading('');
...
HTML;
	...
	if( !@extension_loaded('xml') ) {
		echo "<div class=\"alert alert-warning alert-styled-left alert-arrow-left alert-component\">{$lang['stat_not_min']} XML</div>";
	}
	...
	if( preg_match('/1|yes|on|true/i', ini_get('allow_url_include')) ) {
		echo "<div class=\"alert alert-warning alert-styled-left alert-arrow-left alert-component\">{$lang['stat_secfault_3']}</div>";
	}
	...
	if( $config['cache_type'] ) {
		if ($mcache->connection < 1) {
			...
			} else echo "<div class=\"alert alert-error\">{$lang['stat_m_fail_1']}</div>";
		}
	}
	
	if( function_exists( "get_magic_quotes_gpc" ) && get_magic_quotes_gpc() ) {
		echo "<div class=\"alert alert-warning alert-styled-left alert-arrow-left alert-component\">{$lang['stat_magic']}</div>";
	}
