...
if( $_GET['action'] == "on" OR $_GET['action'] == "off" ) {
	...
	$row = $db->super_query( "SELECT id, mysqlenable, mysqldisable FROM " . PREFIX . "_plugins WHERE id='{$id}'" );
...
if( $_GET['action'] == "delete" ) {
	...
	$row = $db->super_query( "SELECT id, mysqldelete FROM " . PREFIX . "_plugins WHERE id='{$id}'" );
...
if( $_REQUEST['action'] == "download" ) {
	...
	while ( $filerow = $db->get_row() ) {
		...
		$files[$filerow['file']][] = array('action' => $filerow['action'], 'searchcode' => $filerow['searchcode'], 'replacecode' => $filerow['replacecode'] );
	}
...
<dleplugin>
	...
	<mysqldelete><![CDATA[{$row['mysqldelete']}]]></mysqldelete>{$x}
</dleplugin>
HTML;
...
if( $_REQUEST['action'] == "doadd" OR $_REQUEST['action'] == "doedit" ) {
	...
	$icon = $db->safesql( clearfilepath( htmlspecialchars( trim($_POST['icon']), ENT_QUOTES, $config['charset'] ), array ("gif", "jpg", "jpeg", "png" ) ) );
...
if( $_REQUEST['action'] == "add" OR $_REQUEST['action'] == "edit" ) {
	...
	if( $_REQUEST['action'] == "add" ) {
		...
		while ( $filerow = $db->get_row() ) {
			$filerow['file'] = htmlspecialchars( $filerow['file'], ENT_QUOTES, $config['charset'] );
			$files[$filerow['file']][] = array('action' => htmlspecialchars($filerow['action'], ENT_QUOTES, $config['charset'] ), 'searchcode' => htmlspecialchars($filerow['searchcode'], ENT_QUOTES, $config['charset'] ), 'replacecode' => htmlspecialchars($filerow['replacecode'], ENT_QUOTES, $config['charset'] ) );
		}
                ...
                <div class="tab-pane active" id="tabhome">
					<div class="panel-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
							    <input type="text" class="form-control width-500 position-left" name="name" maxlength="100" value="{$row['name']}"><i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right" data-rel="popover" data-trigger="hover" data-placement="right" data-content="{$lang['plugins_name_h']}"></i>
							</div>	
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
							    <input type="text" class="form-control width-500 position-left" name="icon" maxlength="255" value="{$row['icon']}"><i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right" data-rel="popover" data-trigger="hover" data-placement="right" data-content="{$lang['plugins_icon_h']}"></i>
							</div>	
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
							    <input type="text" class="form-control position-left" name="dleversion" maxlength="10" value="{$row['dleversion']}" style="width:100px;">{$versioncompare}<i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right" data-rel="popover" data-trigger="hover" data-placement="right" data-content="{$lang['plugins_dleverh']}"></i>
							</div>	
						</div>
...
HTML;
	...
	$db->query( "SELECT * FROM " . PREFIX . "_plugins ORDER BY id DESC" );
	...
	while ( $row = $db->get_row() ) {
		...
		if( $row['version'] ) $row['version'] = " v.". $row['version'];
		
		if( $row['active'] ) {
			$status = "<span title=\"{$lang['plugins_on_1']}\" class=\"text-success position-right tip\"><b><i class=\"fa fa-check-circle\"></i></b></span>";
			$lang['led_active'] = $lang['plugins_off'];
			$led_action = "off";
		} else {
			$status = "<span title=\"{$lang['plugins_off_1']}\" class=\"text-danger position-right tip\"><b><i class=\"fa fa-exclamation-circle\"></i></b></span>";
			$lang['led_active'] = $lang['plugins_on'];
			$led_action = "on";
		}
		
		if ($plugins_errors[$row['id']]) {
			$plugin_error = "<span class=\"label label-danger position-right\">{$lang['plugins_errors']}</span>";
...
HTML;
		...
		$entries .= "
			<tr>
			 <td class=\"{$border}cursor-pointer\" onclick=\"document.location = '?mod=plugins&action=edit&id={$row['id']}'; return false;\"><div class=\"media-list\"><div class=\"media-left\"><img src=\"{$row['icon']}\" class=\"img-lg section_icon\"></div><div class=\"media-body\"><h6 class=\"media-heading text-semibold\">{$row['name']}{$row['version']}{$status}{$plugin_error}</h6><span class=\"text-muted text-size-small\">{$row['description']}</span></div></div></td>
