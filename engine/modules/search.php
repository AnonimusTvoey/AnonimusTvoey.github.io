		...
		if( (empty( $story ) or ($story_maxlen < $config['search_length_min'])) and (empty( $searchuser ) or (strlen( $searchuser ) < $config['search_length_min'])) ) {
			...
			if( !empty( $stop_not_allowed_list ) ) {
				...
				if( $config['allow_multi_category'] ) {
					...
					$where_category = "category NOT REGEXP '[[:<:]](" . $stop_not_allowed_list . ")[[:>:]]'";
			...
			if( in_array( $titleonly, array (0, 3) ) ) {
				$where_posts = "WHERE " . PREFIX . "_post.approve=1 AND " . PREFIX . "_post_extras.disable_search=0" . $this_date;
			...
			if( $titleonly == 1) {
				$where_comms = "WHERE " . PREFIX . "_post.approve=1  AND " . PREFIX . "_post_extras.disable_search=0" . $this_date;
				...
				$sql_from = "FROM " . PREFIX . "_comments LEFT JOIN " . PREFIX . "_post ON " . PREFIX . "_comments.post_id=" . PREFIX . "_post.id LEFT JOIN " . PREFIX . "_post_extras ON (" . PREFIX . "_comments.post_id=" . PREFIX . "_post_extras.news_id) LEFT JOIN " . USERPREFIX . "_users ON " . PREFIX . "_comments.user_id=" . USERPREFIX . "_users.user_id";
				$sql_fields = $comms_fields;
				$sql_find = "$sql_fields $sql_from $where";
				$sql_from = "FROM " . PREFIX . "_comments LEFT JOIN " . PREFIX . "_post ON " . PREFIX . "_comments.post_id=" . PREFIX . "_post.id LEFT JOIN " . PREFIX . "_post_extras ON (" . PREFIX . "_comments.post_id=" . PREFIX . "_post_extras.news_id)";
