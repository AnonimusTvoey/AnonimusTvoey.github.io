	...
	while ( $row = $db->get_row( $sql_result ) ) {
		...
		if( $view_template == "rss" ) {
			...
			if( $config['rss_format'] != 1 ) {
				...
				$tpl->copy_template = preg_replace( "#\<\!\[CDATA\[(.*?)\{short\-story\}(.*?)\]\]\>#i", "<![CDATA[\\1".$row['short_story']."\\2]]>", $tpl->copy_template );
			...
			$tpl->copy_template = preg_replace( "#\<\!\[CDATA\[(.*?)\{full\-story\}(.*?)\]\]\>#i", "<![CDATA[\\1".$row['full_story']."\\2]]>", $tpl->copy_template );
